...
let W=0,H=0,D=1,run=1,score=0,kills=0,lv=1,xp=0,pending=0,choosing=0,choiceIndex=0,lt=0,ls=0,t=0,est=0,wst=0,pst=0,lgt=0,met=0,orb=0,dtm=0,dcd=0,boost=0,inv=0,berserk=0,surge=0,speedBoost=0,ghost=0,freeze=0,deadeye=0,weaponTime=0,badW=0,shotN=0,rx=0,prismBudget=0,focus=1,focusDelay=0,focusExhaust=0,focusActive=0,shockCD=0,menuUnlockAt=0;

let E=[],B=[],WP=[],P=[],C=[],EX=[],L=[],PT=[],M=[],BH=[],SW=[],jx=0,jy=0,jactive=0,jid=null,jcx=0,jcy=0;

const K={},menuBlocked=new Set(),MAXC=145,hi=new Image();hi.src="hippo.png";
...
singularity:{name:"Singularity Gun",type:"singularity",damage:0,fireRate:1800,bulletSpeed:560,bulletSize:15,pullRadius:350,pullStrength:1100,blastRadius:225,blastDamage:155,blackHoleTime:950,color:"#6d3cff",tier:4,range:9999,visual:"singularity"},
...
const menuKeys=new Set(["w","a","s","d","arrowup","arrowdown","arrowleft","arrowright"," ","enter","e"]);
...
if(performance.now()<menuUnlockAt){e.preventDefault();return}
if(menuBlocked.has(k)){e.preventDefault();return}
...
dbtn.addEventListener("pointerdown",e=>{
...
const hit=(a,b)=>{
let r=rr(a)+rr(b),dx=a.x-b.x,dy=a.y-b.y;
...
function UL(k){
return Math.min(5,(UP[k]||0)+(surge>0?2:0))
}

function need(n=lv){
return Math.floor(8+n*3.7+n*n*.45)
}
...
return r==="common"?10:r==="uncommon"?8:r==="rare"?6.5:r==="epic"?5:3.8
}
...
function maxed(){
return Object.values(UP).every(v=>v>=5)
}
...
function pickUpgrade(pool){
if(!pool.length)return null;

let a=pool.map(k=>({
k,
w:rarityWeight(UD[k].r)*(UP[k]?1.45:1)
}));
...
for(const pool of[weapon,ability,passive]){
let k=pickUpgrade(pool.filter(q=>!pick.includes(q)));
if(k)pick.push(k)
}
...
d.onclick=()=>{
if(performance.now()<menuUnlockAt)return;
chooseUp(k)
};
...
[...cards.children].forEach((q,i)=>q.classList.toggle("selected",i===choiceIndex))
}

function moveChoice(n){
let a=cards.children.length;

if(a){
choiceIndex=(choiceIndex+n+a)%a;
refreshChoice()
}
}
...
let q=cards.children[choiceIndex];

if(q)chooseUp(q.dataset.key)
}
...
let a=Object.entries(G)
.filter(([k])=>k!=="pistol"&&k!==pl.weaponKey)
.map(([k,g])=>{
...
function hpScale(){
return 1+lv*.025+Math.max(0,lv-20)*.012
}
...
hitCooldown:0,slowTimer:0,burnTime:0,burnTick:0,burnStacks:0,
dead:0,bladeHit:0
})
}
...
WP.push({
x:p.x,y:p.y,
radius:23,hitRadius:31,
weaponKey:k,
life:15000,maxLife:15000
})
}
...
P.push({
x:p.x,y:p.y,
radius:19,hitRadius:28,
key:k,
life:18000,maxLife:18000,
pulse:0
})
}
...
cx=a+(Math.random()-.5)*12;
cy=b+(Math.random()-.5)*12
}
...
function critMul(){
return deadeye>0&&Math.random()<.28?3:1
}

function dmgMul(){
return(1+UL("damage")*.15)*(berserk>0?2.5:1)
}

function fireRate(w){
return Math.max(20,w.fireRate/(1+UL("haste")*.1)/(berserk>0?3:1))
}
...
vx:Math.cos(A)*S,
vy:Math.sin(A)*S,
radius:2+Math.random()*5*s,
life:450+Math.random()*500,
color:Math.random()<.5?"#7a0000":"#d00000",
gravity:110
})
}
}
...
pl.health=Math.min(pl.maxHealth,pl.health+h)
}else if(v>=5){
pl.tempHP=Math.min(25,pl.tempHP+h)
}
}
...
function ricEligible(b){
return["normal","plasma","railgun","frost","homing","shotgun","nova","explosive"].includes(b.type)
}
...
if(b.type==="prism"||b.type==="prismShard")return-1;
...
if(b.explosionDamage){
b.explosionDamage*=.65
}
...
let used=new Set(exclude),front=[{e:start,d:damage,depth:0}],total=0,cap=16;
...
let dist=Math.hypot(e.x-f.e.x,e.y-f.e.y);

if(dist<range&&dist<bd){
bd=dist;
q=e
}
}
...
let m=UL("multishot"),beams=1+m,used=new Set();
...
case"shotgun":{
let n=w.pellets+m*3,sp=w.spread+m*.035;

for(let i=0;i<n;i++){
cb(a+(Math.random()-.5)*sp,w)
}
break
}
...
for(let i=0;i<n;i++){
let q=fn(pl,u,1,w.range);

if(q){
u.add(q);
cb(Math.atan2(q.y-pl.y,q.x-pl.x),w)
}else{
cb(a+(i-(n-1)/2)*.12,w)
}
}
...
case"flame":{
let n=1+m,sp=.18+m*.08;

for(let i=0;i<n;i++){
let off=n===1?0:(i/(n-1)-.5)*sp;
...
cb(a+(i-(n-1)/2)*.13,w,{
damage:w.damage*(extra?.78:1),
explosionDamage:w.explosionDamage*(extra?.76:1)
})
}
break
}
...
if(now-ls<fireRate(w)||!E.some(e=>!e.dead&&ons(e)))return;
...
let cr=UL("chainreaction"),gen=b.prismGen||0;

if((b.type==="prismShard"&&gen>=Math.min(5,cr))||prismBudget>=120)return;
...
BH.push({
x:a,y:b,
life:w.blackHoleTime,
duration:w.blackHoleTime,
pullRadius:w.pullRadius,
pullStrength:w.pullStrength,
blastRadius:w.blastRadius,
blastDamage:w.blastDamage,
color:w.color,
pt:0
})
}
...
for(let i=0;i<50;i++){
let A=Math.random()*Math.PI*2,S=160+Math.random()*520;
...
focusActive=1;
focus-=dt/focusDuration();
focusDelay=UP.focus>=5?.65:1;

if(focus<=0){
focus=0;
focusActive=0;
focusExhaust=UP.focus>=5?1:1.5
}
}else{
focusActive=0;
...
focus=Math.min(1,focus+dt/(UP.focus>=5?3.4:4.2))
}
}
}
...
if(time<=0||time>3)return 1;

let rate=time<1?22:10+(3-time)*4;

return .72+.28*(.5+.5*Math.sin(performance.now()/1000*Math.PI*2*rate))
}
...
if(e.burnTick<=0){
e.burnTick=200;
de(e,2.5+e.burnStacks*1.8,0,0,"#ff5a1f","burn")
}

if(Math.random()<dt*16){
PT.push({
x:e.x+(Math.random()-.5)*e.radius,
y:e.y+(Math.random()-.2)*e.radius,
vx:(Math.random()-.5)*25,
vy:-40-Math.random()*80,
radius:2+Math.random()*4,
life:300+Math.random()*300,
color:Math.random()<.5?"#ff7800":"#ffd24a",
gravity:-20
})
}
}else{
...
let b=B[i],spd=Math.hypot(b.vx,b.vy);
...
if(b.type==="explosive"){
boom(
Math.max(0,Math.min(W,b.x)),
Math.max(0,Math.min(H,b.y)),
b.explosionRadius,
b.explosionDamage,
300,
b.color,
...
e.burnStacks=Math.min(5,e.burnStacks+1);
e.burnTick=0
}
...
x:q.x+Math.cos(A)*R,
y:q.y+Math.sin(A)*R,
vx:-Math.cos(A)*(230+Math.random()*280),
vy:-Math.sin(A)*(230+Math.random()*280),
...
for(const ty of["mega","tank","big"]){
let a=E.filter(e=>!e.dead&&ons(e)&&e.type===ty&&!ex.has(e));

if(a.length){
return a.sort((a,b)=>b.health-a.health)[0]
}
}

return E
.filter(e=>!e.dead&&ons(e)&&!ex.has(e))
...
let s=meteorStats(n),lead=.35+n*.045;

let tx=Math.max(s.r,Math.min(W-s.r,q.x+q.vx*lead));
let ty=Math.max(s.r,Math.min(H-s.r,q.y+q.vy*lead));
...
if(met<Math.max(700,2300-n*260))return;

met=0;

let count=n>=5?2:1,used=new Set();
...
e.vx+=dx/d*m.knock*f*res;
e.vy+=dy/d*m.knock*f*res
}
}
...
if(q.electric){
L.push({
x1:q.x+dx/ddd*q.radius,
y1:q.y+dy/ddd*q.radius,
x2:e.x,
y2:e.y,
life:140,
color:"#8fffff"
...
else if(q.key==="shield"){
pl.shield=Math.min(100,pl.shield+50)
}
else if(q.key==="freeze"){
freeze=3
}
else if(q.key==="speedboost"){
speedBoost=8
}
else if(q.key==="deadeye"){
deadeye=10
}
else if(q.key==="ghost"){
ghost=5
}
else if(q.key==="berserk"){
berserk=14
}
else if(q.key==="surge"){
surge=6
...
x.globalAlpha=.2*activePulse(speedBoost);
x.fillStyle="#58ff9c";

x.beginPath();
x.arc(pl.x,pl.y,pl.radius+18,0,Math.PI*2);
x.fill();
...
if(berserk>0){
let p=activePulse(berserk);

x.save();

x.globalAlpha=.4*p;
x.shadowBlur=35;
x.shadowColor="#ff2400";
x.strokeStyle="#ff2400";
x.lineWidth=6;

x.beginPath();
x.arc(pl.x,pl.y,pl.radius+15+5*p,0,Math.PI*2);
x.stroke();
...
x.beginPath();
x.arc(pl.x,pl.y,pl.radius+18+7*p,0,Math.PI*2);
x.stroke();
...
x.beginPath();
x.arc(pl.x,pl.y,pl.radius+12,0,Math.PI*2);
x.fill();
...
x.beginPath();
x.arc(pl.x,pl.y,pl.radius+13,0,Math.PI*2);
x.stroke();
...
x.strokeStyle=surge>0?"#fff4a0":"#fff";
x.lineWidth=3;
x.stroke();
...
x.globalAlpha=.25+.15*Math.sin(performance.now()/60);
x.fillStyle="#ff5a00";

x.shadowBlur=20;
x.shadowColor="#ff5a00";

x.beginPath();
x.arc(e.x,e.y,e.radius+5,0,Math.PI*2);
x.fill();
...
x.globalAlpha=.35*activePulse(freeze);
x.fillStyle="#78dcff";
...
x.drawImage(hi,e.x-s/2,e.y-s/2,s,s)
}else{
...
x.fillStyle="#fff";
x.fillRect(-40,-2,80,4);

x.globalAlpha=.45;
x.fillStyle=b.color;
x.fillRect(-54,-5,108,10)
}
...
x.arc(
b.x,b.y,
b.radius+9+Math.sin(performance.now()/70)*3,
0,Math.PI*2
);
...
x.beginPath();
x.arc(b.x,b.y,b.radius+10,0,Math.PI*2);
x.stroke()
}
...
x.shadowBlur=
(["surge","nuke","berserk"].includes(p.key)?30:16)
+(1-q)*25;
...
function abilityHUD(){
focusFill.style.width=`${focus*100}%`;
focusVal.textContent=`${Math.round(focus*100)}%`;
...
if(UP.shockwave<=0){
shockVal.textContent="LOCKED";
shockVal.className="abilityVal"
}else{
shockVal.textContent=
shockCD<=0
?"READY"
:`${Math.max(0,shockCD).toFixed(1)}s`;
...
if(dbtn){
dbtn.style.opacity=dcd<=0?"1":".45";
dbtn.textContent=dcd<=0?"DASH":dcd.toFixed(1)
}
}
...
function loop(n){
let real=Math.min((n-lt)/1000,.05);

lt=n;

updateFocus(real);

let worldScale=focusActive?.35:1;
let playerScale=focusActive?.58:1;
let world=real*worldScale;
