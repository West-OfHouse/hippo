...
berserk:{name:"BERSERK",color:"#ff2400",w:5},
nuke:{name:"NUKE",color:"#fff",w:.7},
surge:{name:"POWER SURGE",color:"#ffd700",w:3}
};
...
function joyStart(e){
 ...
 let p=[...e.changedTouches].find(t=>t.clientX<W*.58);
 if(!p)return;
...
function joyMove(e){
 if(!jactive)return;
 let p=[...e.touches].find(t=>t.identifier===jid);
 if(!p)return;
 e.preventDefault();
 let dx=p.clientX-jcx,dy=p.clientY-jcy,d=Math.hypot(dx,dy),m=42;
...
function joyEnd(e){
 ...
 if([...e.changedTouches].some(t=>t.identifier===jid)){
  jactive=0;jid=null;jx=jy=0;
  jb.style.display="none";js.style.transform="translate(0,0)"
 }
}
...
function sw(){WP.length=0;let k=weaponChoice();if(!k)return;let p=rp();WP.push({x:p.x,y:p.y,radius:23,weaponKey:k,life:15000})}
...
function kill(e,src=""){if(!e||e.dead)return;e.dead=1;kills++;score+=10;blood(e.x,e.y,e.radius/22);sc(e.x,e.y,e.xpv);let v=UL("vampire");if(v){let h=.6*v;if(pl.health<pl.maxHealth)pl.health=Math.min(pl.maxHealth,pl.health+h);else if(v>=5)pl.tempHP=Math.min(25,pl.tempHP+h)}let r=UL("chainreaction");if(r&&(src!=="reaction"||r>=5)&&rx<26){rx++;boom(e.x,e.y,48+r*13,24+r*12,210,"#ff3c6f","reaction")}}
function de(e,d,k=0,a=0,col="#fff",src=""){if(!e||e.dead)return;let actual=d*dmgMul();if(UL("damage")>=5&&e.health/e.maxHealth<.2){kill(e,src);return}e.health-=actual;if(k){let resist=e.type==="mega"?.32:e.type==="tank"?.48:e.type==="big"?.68:1;e.vx+=Math.cos(a)*k*2.5*resist;e.vy+=Math.sin(a)*k*2.5*resist}hp(e.x,e.y,col);if(e.health<=0)kill(e,src)}
...
function chain(s,d,n,r,col,ex=new Set()){let f={x:s.x,y:s.y},h=new Set(ex);h.add(s);for(let i=0;i<n;i++){let q=null,bd=1e9;for(const e of E){if(e.dead||h.has(e)||!ons(e))continue;let z=Math.hypot(e.x-f.x,e.y-f.y);if(z<r&&z<bd){bd=z;q=e}}if(!q)break;L.push({x1:f.x,y1:f.y,x2:q.x,y2:q.y,life:135,color:col});de(q,d,0,0,col);h.add(q);f=q;d*=.95}}

function arcBeam(e,w,used=new Set()){let cr=UL("chainreaction"),q=e,d=w.damage,f={x:pl.x,y:pl.y},h=new Set(used),chains=w.chains+cr*2,range=w.chainRange+cr*18;for(let i=0;i<chains&&q;i++){L.push({x1:f.x,y1:f.y,x2:q.x,y2:q.y,life:150,color:w.color});de(q,d,0,0,w.color);h.add(q);used.add(q);f=q;d=i?d*.95:w.chainDamage;let z=null,bd=1e9;for(const p of E){if(p.dead||h.has(p)||!ons(p))continue;let u=Math.hypot(p.x-f.x,p.y-f.y);if(u<range&&u<bd){bd=u;z=p}}q=z}}
function arcVolley(w){let ml=UL("multishot"),beams=Math.max(1,ml),used=new Set();for(let i=0;i<beams;i++){let q=fn(pl,used,1,w.range);if(!q)break;arcBeam(q,w,used)}}
...
function prism(b,e){
 ...
 let cr=UL("chainreaction"),maxGen=Math.min(5,cr),gen=b.prismGen||0;if(b.type==="prismShard"&&gen>=maxGen||prismBudget>=100)return;
 ...
 for(let i=0;i<n;i++){let f=n===1?0:i/(n-1)-.5,a=base+f*(gen===0?b.splitSpread:1.05+gen*.12)+(Math.random()-.5)*.11;cb(a,G.prism,{x:e.x,y:e.y,damage:b.damage*(gen===0?.48:.62),type:"prismShard",visual:"prismShard",color:cols[(i+gen)%cols.length],speed:1030+Math.random()*220,radius:4,life:650,postLimit:230,prismGen:gen+1})}
}
...
function up(dt){
 ...
 if(dtm>0){dtm-=dt;pl.x+=pl.dvx*dt;pl.y+=pl.dvy*dt}
  ...
  if(d>.08){a/=Math.max(1,d);b/=Math.max(1,d);pl.lastDX=a;pl.lastDY=b;let strength=Math.min(1,d);pl.x+=a*s*strength*dt;pl.y+=b*s*strength*dt}
 }
...
function wall(b){
 let hitWall=0;
 if(b.x-b.radius<0&&b.vx<0){b.x=b.radius;b.vx*=-1;hitWall=1}
 if(b.x+b.radius>W&&b.vx>0){b.x=W-b.radius;b.vx*=-1;hitWall=1}
 if(b.y-b.radius<0&&b.vy<0){b.y=b.radius;b.vy*=-1;hitWall=1}
 if(b.y+b.radius>H&&b.vy>0){b.y=H-b.radius;b.vy*=-1;hitWall=1}
...
function ub(dt){
 for(let i=B.length-1;i>=0;i--){
  let b=B[i],spd=Math.hypot(b.vx,b.vy);b.life-=dt*1000;if(b.postRic){b.postTravel+=spd*dt;if(b.postTravel>=b.postLimit){B.splice(i,1);continue}}
  b.x+=b.vx*dt;b.y+=b.vy*dt;

  if((UL("ricochet")||b.type==="prismShard")&&["normal","railgun","frost","homing","prism","prismShard","nova","explosive"].includes(b.type)){
   if(!wall(b)){if(b.type==="explosive")boom(b.x,b.y,b.explosionRadius,b.explosionDamage,300,b.color);B.splice(i,1);continue}
  }else if(b.x<-180||b.x>W+180||b.y<-180||b.y>H+180){if(b.type==="singularity")singularity(b.x,b.y);B.splice(i,1);continue}
  ...
  for(const e of E){
   ...
   if(b.ric>0){
    ...
    if(q){let a=Math.atan2(q.y-e.y,q.x-e.x),s=Math.hypot(b.vx,b.vy);b.vx=Math.cos(a)*s;b.vy=Math.sin(a)*s;b.angle=a;consumeRic(b);break}
   }
   rm=1;break
  }
  if(rm)B.splice(i,1)
...
function ubh(dt){for(let i=BH.length-1;i>=0;i--){let q=BH[i];q.life-=dt*1000,p=1-q.life/q.duration;for(const e of E){let dx=q.x-e.x,dy=q.y-e.y,d=Math.hypot(dx,dy);if(d>2&&d<q.pullRadius){let f=q.pullStrength*(1-d/q.pullRadius)*(1+p)*dt;e.vx+=dx/d*f;e.vy+=dy/d*f}}if(q.life<=0){singularityBoom(q);BH.splice(i,1)}}}
...
function us(dt){
 ...
 for(let i=0;i<count;i++){
  ...
  for(const e of E)if(!e.dead&&hit(q,e)&&performance.now()-e.bladeHit>220){e.bladeHit=performance.now();let dx=e.x-pl.x,dy=e.y-pl.y,d=Math.hypot(dx,dy)||1;e.vx+=dx/d*kb;e.vy+=dy/d*kb;de(e,damage,0,0,"#ffda44","solar")}
 }
}

function ul(dt){let s=UL("lightning");if(!s)return;lgt+=dt*1000;if(lgt<1800-s*180)return;lgt=0;for(let j=0;j<(s>=5?3:1);j++){let q=fn(pl,new Set(),1,Infinity);if(!q)break;de(q,35+s*12,0,0,"#68f7ff");chain(q,(35+s*12)*.75,1+s,140+s*20,"#68f7ff")}}
...
function um(dt){let s=UL("meteors");if(!s)return;met+=dt*1000;megaMet+=dt;if(met>=Math.max(650,2100-s*260)){met=0;M.push({x:50+Math.random()*(W-100),y:70+Math.random()*(H-140),life:650,duration:650,radius:34+s*7,damage:50+s*22})}if(s>=5&&megaMet>=10){megaMet=0;let q=densest();if(q)M.push({x:q.x,y:q.y,life:900,duration:900,radius:150,damage:400,mega:1})}}
...
function upp(dt){
 ...
 for(let i=P.length-1;i>=0;i--){let q=P[i];q.life-=dt*1000;q.pulse+=dt*5;if(q.life<=0){P.splice(i,1);continue}if(hit(pl,q)){if(q.key==="heal")pl.health=pl.maxHealth;else if(q.key==="shield")pl.shield=Math.min(100,pl.shield+50);else if(q.key==="berserk")berserk=14;else if(q.key==="surge")surge=6;else if(q.key==="nuke"){for(const e of E)if(!e.dead&&ons(e))kill(e);for(const z of C)z.nukeVacuum=1}P.splice(i,1)}}
}
...
function uc(dt){let ms=magnetStats();for(let i=C.length-1;i>=0;i--){let q=C[i];q.life-=dt*1000;q.x+=q.vx*dt;q.y+=q.vy*dt;q.vx*=.94;q.vy*=.94;if(q.nukeVacuum)pull(q,dt,99999,1150);else pull(q,dt,ms.r,ms.s);if(q.life<=0)C.splice(i,1);else if(hit(pl,q)){gain(q.value);C.splice(i,1)}}}

function ufx(dt){for(let i=PT.length-1;i>=0;i--){let p=PT[i];p.life-=dt*1000;p.x+=p.vx*dt;p.y+=p.vy*dt;if(p.life<=0)PT.splice(i,1)}for(let i=EX.length-1;i>=0;i--){EX[i].life-=dt*1000;if(EX[i].life<=0)EX.splice(i,1)}for(let i=L.length-1;i>=0;i--){L[i].life-=dt*1000;if(L[i].life<=0)L.splice(i,1)}}
...
function dp(){if(pl.shield>0){x.beginPath();x.arc(pl.x,pl.y,pl.radius+8,0,Math.PI*2);x.strokeStyle="#55cfff";x.lineWidth=4;x.stroke()}x.beginPath();x.arc(pl.x,pl.y,pl.radius,0,Math.PI*2);x.fillStyle=berserk>0?"#ff4b32":pl.color;x.fill();x.strokeStyle="#fff";x.lineWidth=3;x.stroke()}

function dee(){for(const e of E){let s=e.radius*2.65,z=ET[e.type];if(z.outline){x.save();x.strokeStyle=z.outline;x.lineWidth=e.type==="mega"?7:e.type==="tank"?6:3;x.shadowBlur=12;x.shadowColor=z.outline;x.beginPath();x.arc(e.x,e.y,e.radius+7,0,Math.PI*2);x.stroke();x.restore()}if(hi.complete&&hi.naturalWidth)x.drawImage(hi,e.x-s/2,e.y-s/2,s,s);if(e.elite||e.health<e.maxHealth){let w=e.radius*1.8;x.fillStyle="#250000";x.fillRect(e.x-w/2,e.y-e.radius-10,w,5);x.fillStyle="#f44";x.fillRect(e.x-w/2,e.y-e.radius-10,w*e.health/e.maxHealth,5)}}}

function dc(){for(const q of C){x.fillStyle="#ffd700";x.beginPath();x.arc(q.x,q.y,q.radius,0,Math.PI*2);x.fill()}}
function db(){for(const b of B){x.save();x.fillStyle=b.color;x.shadowBlur=12;x.shadowColor=b.color;if(b.visual==="prism"||b.visual==="prismShard"){x.translate(b.x,b.y);x.rotate(b.angle);x.beginPath();x.moveTo(12,0);x.lineTo(-7,-5);x.lineTo(-7,5);x.fill()}else{x.beginPath();x.arc(b.x,b.y,b.radius,0,Math.PI*2);x.fill()}x.restore()}}
function ds(){let n=UL("solar");if(!n)return;let count=Math.min(5,n+1),r=n>=5?82:70+n*2;for(let i=0;i<count;i++){let a=orb+Math.PI*2/count*i,A=pl.x+Math.cos(a)*r,Bb=pl.y+Math.sin(a)*r;x.save();x.translate(A,Bb);x.rotate(a);x.fillStyle="#fff7a0";x.shadowBlur=18;x.shadowColor="#ffda44";x.fillRect(-4,-16,8,32);x.restore()}}
function dwp(){for(const p of WP){let w=G[p.weaponKey];x.beginPath();x.arc(p.x,p.y,p.radius,0,Math.PI*2);x.fillStyle="#111";x.fill();x.strokeStyle=w.color;x.lineWidth=4;x.stroke();x.fillStyle="#fff";x.font="bold 8px Arial";x.textAlign="center";x.fillText(w.name.split(" ")[0],p.x,p.y+3)}}
function dpu(){for(const p of P){let u=PU[p.key];x.beginPath();x.arc(p.x,p.y,p.radius,0,Math.PI*2);x.fillStyle=u.color;x.fill();x.fillStyle="#111";x.font="bold 8px Arial";x.textAlign="center";x.fillText(u.name,p.x,p.y+3)}}
function dmet(){for(const m of M){x.beginPath();x.arc(m.x,m.y,m.radius,0,Math.PI*2);x.strokeStyle="#ff6b35";x.lineWidth=4;x.stroke()}}
function dbh(){for(const q of BH){x.beginPath();x.arc(q.x,q.y,24,0,Math.PI*2);x.fillStyle="#030005";x.fill();x.strokeStyle="#b98cff";x.lineWidth=4;x.stroke()}}
function dfx(){for(const p of PT){x.globalAlpha=Math.min(1,p.life/250);x.beginPath();x.arc(p.x,p.y,p.radius,0,Math.PI*2);x.fillStyle=p.color;x.fill();x.globalAlpha=1}for(const l of L){x.strokeStyle=l.color;x.lineWidth=4;x.beginPath();x.moveTo(l.x1,l.y1);x.lineTo(l.x2,l.y2);x.stroke()}}

function hud(){ht.textContent=`HP ${Math.ceil(pl.health)}`;wt.textContent=pl.weaponKey==="pistol"?"Pistol":`${gw().name} ${Math.ceil(weaponTime/1000)}s`;st.textContent=`Score ${score}`;lvT.textContent=`LV ${lv}`;xpB.style.width=`${Math.min(100,xp/need()*100)}%`;if(pl.shield>0){shHUD.classList.remove("hidden");shT.textContent=Math.ceil(pl.shield)}else shHUD.classList.add("hidden")}
...
function reset(){
 ...
 ls=est=wst=pst=lgt=met=megaMet=orb=dtm=dcd=0;run=1;choosing=0;
 go.classList.add("hidden");lscr.classList.add("hidden");sw();sp();lt=performance.now()
}
rb.onclick=reset;
