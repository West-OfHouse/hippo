...
singularity:{name:"Singularity Gun",type:"singularity",damage:0,fireRate:1750,bulletSpeed:420,bulletSize:15,pullRadius:330,pullStrength:900,blastRadius:215,blastDamage:150,blackHoleTime:850,color:"#6d3cff",tier:3,range:450,visual:"singularity"},
...
prism:{name:"Prism Cannon",type:"prism",damage:82,fireRate:650,bulletSpeed:1050,bulletSize:6,splits:8,splitSpread:1.25,color:"#ff74e8",tier:5,range:610,visual:"prism"}
};

const U={
heal:{name:"HEAL",color:"#40ff75",w:19,d:0},
shield:{name:"SHIELD",color:"#5ac8ff",w:17,d:0},
magnet:{name:"MAGNET",color:"#c451ff",w:13,d:20000,m:2},
haste:{name:"HASTE",color:"#ffe600",w:11,d:20000,m:2},
berserk:{name:"BERSERK",color:"#ff0055",w:10,d:20000,m:2},
vampire:{name:"VAMPIRE",color:"#a80038",w:9,d:20000,m:2},
fmj:{name:"FMJ",color:"#c8c8c8",w:9,d:20000,m:2},
ricochet:{name:"RICOCHET",color:"#b6ff5c",w:8,d:20000,m:2},
quad:{name:"DAMAGE",color:"#ff3838",w:8,d:20000,m:2},
chain:{name:"CHAIN",color:"#00ffff",w:8,d:20000,m:2},
meteors:{name:"METEORS",color:"#ff6b35",w:7,d:20000,m:2},
overdrive:{name:"OVERDRIVE",color:"#ff7b00",w:6,d:20000,m:2},
lightning:{name:"LIGHTNING",color:"#68f7ff",w:6,d:20000,m:2},
multishot:{name:"MULTI",color:"#ff8cff",w:5,d:20000,m:2},
chainreaction:{name:"REACTION",color:"#ff3c6f",w:5,d:20000,m:2},
nuke:{name:"NUKE",color:"#fff",w:3,d:0},
powersurge:{name:"POWER SURGE",color:"#fff",w:2,d:5500,m:1}
};
...
function weaponWeights(){let late=Math.min(1,t/150+score/4200),pity=Math.min(1,badWeaponRolls/5);return Object.entries(G).filter(([k])=>k!=="pistol").map(([k,g])=>{let w=1;if(g.tier===1)w=28-8*late;if(g.tier===2)w=26-2*late;if(g.tier===3)w=20+8*late;if(g.tier===4)w=12+10*late;if(g.tier===5)w=7+8*late;w*=1+(g.tier>=3?pity*1.35:0);if(badWeaponRolls>=5&&g.tier<3)w=0;return{k,w}})}
function cw(){let k=wc(weaponWeights());badWeaponRolls=G[k].tier>=3?0:badWeaponRolls+1;return k}
function cp(){let a=[];for(const[k,u]of Object.entries(U)){let w=u.w,b=BF[k];if(b?.stacks)w*=b.stacks>=b.maxStacks?.18:.5;if(k==="heal"&&pl.health>pl.maxHealth*.8)w*=.4;if(k==="shield"&&pl.shield>120)w*=.5;a.push({k,w})}return wc(a)}

function spawnSide(){let d=[{s:0,v:pl.y/H},{s:2,v:1-pl.y/H},{s:3,v:pl.x/W},{s:1,v:1-pl.x/W}].sort((a,b)=>b.v-a.v);if(Math.random()<.82)return d[0].s;return Math.random()<.7?d[1].s:Math.floor(Math.random()*4)}
function se(){let s=spawnSide(),m=55,a,b;if(!s){a=Math.random()*W;b=-m}else if(s===1){a=W+m;b=Math.random()*H}else if(s===2){a=Math.random()*W;b=H+m}else{a=-m;b=Math.random()*H}let q=1+score/850+t/130,r=19+Math.random()*4,v=70+Math.random()*28+q*2.65,dx=pl.x-a,dy=pl.y-b,d=Math.hypot(dx,dy)||1,h=70+q*12;E.push({x:a,y:b,radius:r,vx:dx/d*v*.7,vy:dy/d*v*.7,baseMaxSpeed:v,steering:120+Math.random()*35,health:h,maxHealth:h,damage:17+q*.45,hitCooldown:0,slowTimer:0,burnTime:0,burnTick:0,burnDamage:0,dead:0,bladeHit:0})}

function sw(){let p=rp();WP.push({x:p.x,y:p.y,radius:23,weaponKey:cw(),life:30000})}
function sp(){let p=rp();P.push({x:p.x,y:p.y,radius:19,key:cp(),life:26000,pulse:0})}
...
function singularityActive(){return B.some(b=>b.type==="singularity")||BH.length>0}
...
function shoot(now){
 ...
 if(w.type==="nova"){cb(a,w);let n=w.sideShots+es("overdrive")+m;for(let i=1;i<=n;i++){cb(a-.18*i,w,{visual:"lanceSide"});cb(a+.18*i,w,{visual:"lanceSide"})}return}
 cb(a,w);
...
function ps(b,e){
 ...
 const cols=["#ff4fd8","#72f7ff","#fff16e","#b779ff","#7dff9b","#ff8b4a","#6ca8ff"];
 let n=b.splits+es("multishot")*2,base=b.angle,spread=b.splitSpread||1.25;
 for(let i=0;i<n;i++){
  let f=n===1?0:i/(n-1)-.5,a=base+f*spread+(Math.random()-.5)*.08;
  cb(a,G.prism,{x:e.x+Math.cos(base)*10,y:e.y+Math.sin(base)*10,damage:b.damage*.48,type:"prismShard",visual:"prismShard",color:cols[i%cols.length],speed:1050+Math.random()*180,radius:4,life:800,pierce:0});
 }
}
...
function singularity(a,b){
 ...
 for(let i=0;i<36;i++){let A=Math.random()*Math.PI*2,R=55+Math.random()*w.pullRadius*.9;PT.push({x:a+Math.cos(A)*R,y:b+Math.sin(A)*R,vx:0,vy:0,radius:1.5+Math.random()*2.5,life:w.blackHoleTime+100,color:Math.random()<.5?"#a574ff":"#fff",gravity:0,suckX:a,suckY:b,suckSpeed:330+Math.random()*360,suck:1,spin:(Math.random()<.5?-1:1)*(2+Math.random()*4)})}
}
function singularityBoom(q){
 boom(q.x,q.y,q.blastRadius,q.blastDamage,400,"#b26cff");
 for(let i=0;i<52;i++){let A=Math.random()*Math.PI*2,S=180+Math.random()*520;PT.push({x:q.x,y:q.y,vx:Math.cos(A)*S,vy:Math.sin(A)*S,radius:2+Math.random()*4,life:350+Math.random()*350,color:Math.random()<.5?"#d7b8ff":"#7b33ff",gravity:0})}
 for(const e of E){if(e.dead)continue;let dx=e.x-q.x,dy=e.y-q.y,d=Math.hypot(dx,dy)||1;if(d<q.blastRadius*1.4){let f=240*(1-Math.min(1,d/(q.blastRadius*1.4)));e.vx+=dx/d*f;e.vy+=dy/d*f}}
}
...
function uweap(dt){
 if(pl.weaponKey==="pistol"){weaponTime=0;return}
 weaponTime-=dt*1000;
 if(weaponTime<=0){pl.weaponKey="pistol";weaponTime=0;ls=0}
}
...
function ub(dt){
 for(let i=B.length-1;i>=0;i--){
  let b=B[i];b.life-=dt*1000;b.trail.push({x:b.x,y:b.y,life:180});if(b.trail.length>8)b.trail.shift();
...
function ubh(dt){
 for(let i=BH.length-1;i>=0;i--){
  let q=BH[i];q.life-=dt*1000;q.particleTimer-=dt*1000;let p=1-q.life/q.duration,str=.55+p*1.35;
  for(const e of E){if(e.dead)continue;let dx=q.x-e.x,dy=q.y-e.y,d=Math.hypot(dx,dy);if(d>2&&d<q.pullRadius){let f=q.pullStrength*(1-d/q.pullRadius)*str*dt;e.vx+=dx/d*f;e.vy+=dy/d*f;e.x+=dx/d*f*.3;e.y+=dy/d*f*.3}}
  if(q.particleTimer<=0){q.particleTimer=28;for(let z=0;z<4;z++){let A=Math.random()*Math.PI*2,R=70+Math.random()*q.pullRadius*.85;PT.push({x:q.x+Math.cos(A)*R,y:q.y+Math.sin(A)*R,vx:0,vy:0,radius:1+Math.random()*2.5,life:550,color:Math.random()<.5?"#8d55ff":"#eee4ff",gravity:0,suckX:q.x,suckY:q.y,suckSpeed:420+Math.random()*420,suck:1,spin:(Math.random()<.5?-1:1)*(3+Math.random()*4)})}}
...
function upp(dt){
 ...
 for(let i=P.length-1;i>=0;i--){let q=P[i];q.life-=dt*1000;q.pulse+=dt*5;let m=es("magnet");if(m)pull(q,dt,130+m*25,220+m*35);if(q.life<=0){P.splice(i,1);continue}if(hit(pl,q)){act(q.key);P.splice(i,1)}}
}
...
function db(){
 for(const b of B){
  x.save();
  for(let i=0;i<b.trail.length;i++){let q=b.trail[i];x.globalAlpha=(i+1)/b.trail.length*.3;x.beginPath();x.arc(q.x,q.y,Math.max(1,b.radius*(i+1)/b.trail.length*.7),0,Math.PI*2);x.fillStyle=b.color;x.fill()}x.globalAlpha=1;
...
function dbh(){for(const q of BH){let p=1-q.life/q.duration,r=q.pullRadius*(.35+.65*p);x.save();x.globalAlpha=.35+.35*Math.sin(performance.now()/45);x.shadowBlur=40;x.shadowColor=q.color;x.beginPath();x.arc(q.x,q.y,18+14*p,0,Math.PI*2);x.fillStyle="#050008";x.fill();x.strokeStyle="#b98cff";x.lineWidth=3;x.stroke();x.globalAlpha=.2;x.beginPath();x.arc(q.x,q.y,r,0,Math.PI*2);x.strokeStyle="#8c52ff";x.lineWidth=2;x.stroke();x.restore()}}
...
function hud(){ht.textContent=`HP: ${Math.ceil(pl.health)}`;wt.textContent=pl.weaponKey==="pistol"?"Pistol":`${gw().name} ${Math.ceil(weaponTime/1000)}s`;st.textContent=`Score: ${score} | Coins: ${coins}`}
function end(){run=0;fs.textContent=`Score: ${score} | Coins: ${coins}`;go.classList.remove("hidden")}
function reset(){E=[];B=[];WP=[];P=[];C=[];EX=[];L=[];PT=[];M=[];BH=[];score=0;kills=0;coins=0;t=0;weaponTime=0;badWeaponRolls=0;pl.x=W/2;pl.y=H/2;pl.health=pl.maxHealth;pl.shield=0;pl.weaponKey="pistol";pl.lastDX=1;pl.lastDY=0;for(const k in BF){BF[k].stacks=0;BF[k].time=0}for(const k in K)K[k]=0;ls=est=wst=pst=lgt=met=orb=dtm=dcd=0;pa=0;run=1;go.classList.add("hidden");sw();sw();sp();lt=performance.now()}rb.addEventListener("click",reset);

function loop(n){
 let d=Math.min((n-lt)/1000,.05);lt=n;bg();
 if(run){
  t+=d;est+=d*1000;wst+=d*1000;pst+=d*1000;
  let alive=E.reduce((q,e)=>q+(!e.dead),0),er=Math.max(170,1250-score*.38-t*.9),ec=score>=18000?4:score>=10000?3:score>=4500?2:1;
  if(est>=er){est=0;for(let i=0;i<ec;i++)se()}
  if(wst>=7800){wst=0;sw()}

  let late=Math.min(1,t/180+score/7000);
  let pr=Math.max(3600,10500-Math.min(3200,alive*55)-late*2600);

  if(pst>=pr){
   pst=0;sp();
   if(late>.45&&Math.random()<.18+.18*late)sp();
  }

  up(d);uweap(d);ue(d);ub(d);ubh(d);us(d);ul(d);um(d);umo(d);upp(d);uc(d);ubf(d);ufx(d);shoot(n);hud();
 }
