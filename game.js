...
let W=0,H=0,D=1,run=1,score=0,kills=0,lv=1,xp=0,pending=0,choosing=0,choiceIndex=0,lt=0,ls=0,t=0,est=0,wst=0,pst=0,lgt=0,met=0,megaMet=0,orb=0,dtm=0,dcd=0,boost=0,inv=0,berserk=0,surge=0,speedBoost=0,ghost=0,freeze=0,deadeye=0,weaponTime=0,badW=0,shotN=0,rx=0,prismBudget=0;
...
const UP={fmj:0,multishot:0,ricochet:0,prismup:0,lightning:0,meteors:0,chainreaction:0,vampire:0,haste:0,damage:0,speed:0,health:0,magnet:0,solar:0};
...
ricochet:{n:"RICOCHET",t:"WEAPON MOD",r:"rare",d:"+1 enemy or wall bounce. Damage falls after every bounce.",e:"PINBALL: wall bounces no longer consume charges."},
prismup:{n:"PRISM",t:"WEAPON MOD",r:"rare",d:"Adds more shards when the Prism Cannon fractures.",e:"KALEIDOSCOPE: +12 shards and a much wider fracture spread."},
...
chainreaction:{n:"CHAIN REACTION",t:"ABILITY",r:"epic",d:"Dead hippos explode. Improves Arc and Prism recursion.",e:"CRITICAL MASS: reaction kills explode again."},
solar:{n:"SOLAR BLADES",t:"ABILITY",r:"legendary",d:"Starts with 2 blades. More levels add blades and damage.",e:"SOLAR GUARD: 5 stronger defensive blades."}
};
...
function sw(){WP.length=0;let k=weaponChoice();if(!k)return;let p=rp();WP.push({x:p.x,y:p.y,radius:23,hitRadius:31,weaponKey:k,life:15000})}
function sp(){let k=puChoice();if(!k)return;let p=rp();P.push({x:p.x,y:p.y,radius:19,hitRadius:28,key:k,life:18000,pulse:0})}
...
function de(e,d,k=0,a=0,col="#fff",src=""){
 ...
 if(cm>1){for(let i=0;i<8;i++)PT.push({x:e.x,y:e.y,vx:(Math.random()-.5)*240,vy:(Math.random()-.5)*240,radius:2+Math.random()*3,life:280,color:"#ffb45b",gravity:0})}
...
function arcBeam(e,w,used=new Set()){
 ...
 for(let i=0;i<chains&&q;i++){
  ...
  de(q,d,0,0,w.color,"weapon");h.add(q);used.add(q);f=q;d=i?d*.95:w.chainDamage;
...
function volley(a,w,free=0){
 ...
 if(w.type==="flame"){
  for(let i=0;i<1+ml;i++)cb(a+(Math.random()-.5)*w.spread,w,{type:"flame",life:900,speed:w.bulletSpeed*(.85+Math.random()*.35),radius:w.bulletSize+Math.random()*5});
  return
 }

 if(w.type==="nova"){
  cb(a,w);
  for(let i=1;i<=w.sideShots+ml;i++){cb(a-.18*i,w,{visual:"lanceSide"});cb(a+.18*i,w,{visual:"lanceSide"})}
  return
 }
...
function prism(b,e){
 ...
 let cols=["#ff4fd8","#72f7ff","#fff16e","#b779ff","#7dff9b","#ff8b4a","#6ca8ff","#ff6f91"],base=b.angle;
 let n=gen===0?b.splits+prismBonus()+UL("multishot"):Math.max(2,6-gen+Math.floor(UL("prismup")/2));
 n=Math.min(n,120-prismBudget);prismBudget+=n;

 let spread=gen===0?b.splitSpread+(UL("prismup")>=5?.55:0):1.05+gen*.12;

 for(let i=0;i<n;i++){
  let f=n===1?0:i/(n-1)-.5,a=base+f*spread+(Math.random()-.5)*.11;
  cb(a,G.prism,{x:e.x+Math.cos(base)*10,y:e.y+Math.sin(base)*10,damage:b.damage*(gen===0?.48:.6),type:"prismShard",visual:"prismShard",color:cols[(i+gen)%cols.length],speed:1030+Math.random()*220,radius:Math.max(2.5,4-gen*.2),life:650,postLimit:220,prismGen:gen+1})
 }
}

function singularity(a,b){
 let w=G.singularity;
 BH.push({x:a,y:b,life:w.blackHoleTime,duration:w.blackHoleTime,pullRadius:w.pullRadius,pullStrength:w.pullStrength,blastRadius:w.blastRadius,blastDamage:w.blastDamage,color:w.color,pt:0})
}
...
function singularityBoom(q){
 ...
 for(let i=0;i<45;i++){
  ...
  PT.push({x:q.x,y:q.y,vx:Math.cos(A)*S,vy:Math.sin(A)*S,radius:2+Math.random()*4,life:300+Math.random()*350,color:Math.random()<.5?"#d7b8ff":"#7b33ff",gravity:0})
 }
}
...
function up(dt){
 ...
 if(dtm>0){dtm-=dt;pl.x+=pl.dvx*dt;pl.y+=pl.dvy*dt}
  ...
  let s=pl.speed*(1+UL("speed")*.05)*(boost>0?1.5:1)*(berserk>0?1.35:1)*(speedBoost>0?1.65:1);

  if(d>.08){
   let str=Math.min(1,d);a/=d;b/=d;pl.lastDX=a;pl.lastDY=b;
   pl.x+=a*s*str*dt;pl.y+=b*s*str*dt
  }
 }
...
function ue(dt){
 for(let i=E.length-1;i>=0;i--){
  ...
  if(freeze>0)ms=0;
  else if(e.slowTimer>0){e.slowTimer-=dt*1000;ms*=.5}

  if(e.burnTime>0){
   e.burnTime-=dt*1000;e.burnTick-=dt*1000;
   if(e.burnTick<=0){e.burnTick=250;de(e,e.burnDamage,0,0,"#ff5a1f","weapon")}
  }
  ...
  if(ms>0){
   ...
   e.vx*=Math.pow(.03,dt);e.vy*=Math.pow(.03,dt)
  }
 ...
 for(let i=B.length-1;i>=0;i--){
  ...
  if(b.type==="homing"){
   ...
   if(q){
    ...
    let s=Math.hypot(b.vx,b.vy);b.vx=Math.cos(b.angle)*s;b.vy=Math.sin(b.angle)*s
   }
  }
  ...
  if((UL("ricochet")||b.type==="prismShard")&&["normal","railgun","frost","homing","prism","prismShard","nova","explosive","shotgun"].includes(b.type)){
   if(!wall(b)){
    if(b.type==="explosive")boom(b.x,b.y,b.explosionRadius,b.explosionDamage,300,b.color,"weapon");
  ...
  for(const e of E){
   ...
   if(b.ric>0){
    ...
    if(q){
     ...
     b.vx=Math.cos(a)*s;b.vy=Math.sin(a)*s;b.angle=a;
     consumeRic(b);break
    }
   }
...
function ubh(dt){
 for(let i=BH.length-1;i>=0;i--){
  let q=BH[i];q.life-=dt*1000;q.pt-=dt*1000;
  ...
  if(q.pt<=0){
   ...
   for(let z=0;z<4;z++){
    ...
    PT.push({x:q.x+Math.cos(A)*R,y:q.y+Math.sin(A)*R,vx:-Math.cos(A)*(180+Math.random()*240),vy:-Math.sin(A)*(180+Math.random()*240),radius:1+Math.random()*3,life:350,color:"#b788ff",gravity:0})
   }
  }
...
function us(dt){
 ...
 orb+=dt*4.2;
 let count=Math.min(5,n+1),r=n>=5?82:70+n*2,damage=16+n*6,kb=10+n*4;
 ...
 for(let i=0;i<count;i++){
  ...
  for(const e of E){
   if(!e.dead&&hit(q,e)&&performance.now()-e.bladeHit>220){
    ...
    let dx=e.x-pl.x,dy=e.y-pl.y,d=Math.hypot(dx,dy)||1,res=e.type==="mega"?.3:e.type==="tank"?.45:e.type==="big"?.7:1;
    e.vx+=dx/d*kb*res;
    e.vy+=dy/d*kb*res;
...
function ul(dt){
 ...
 lgt+=dt*1000;
 if(lgt<1800-s*180)return;
 lgt=0;

 for(let j=0;j<(s>=5?3:1);j++){
  let q=fn(pl,new Set(),1,Infinity);if(!q)break;
  L.push({x1:q.x+(Math.random()-.5)*60,y1:-20,x2:q.x,y2:q.y,life:180,color:"#68f7ff"});
  de(q,35+s*12,0,0,"#68f7ff");
  chain(q,(35+s*12)*.75,1+s,140+s*20,"#68f7ff")
 }
}
...
function um(dt){
 ...
 if(met>=Math.max(650,2100-s*260)){
  met=0;
  M.push({x:50+Math.random()*(W-100),y:70+Math.random()*(H-140),life:650,duration:650,radius:34+s*7,damage:50+s*22})
 }

 if(s>=5&&megaMet>=10){
  megaMet=0;
  let q=densest();
  if(q)M.push({x:q.x,y:q.y,life:900,duration:900,radius:150,damage:400,mega:1})
 }
}

function umo(dt){
 for(let i=M.length-1;i>=0;i--){
  let m=M[i];m.life-=dt*1000;
  if(m.life<=0){
   boom(m.x,m.y,m.radius,m.damage,m.mega?500:300,m.mega?"#fff2a8":"#ff6b35");
   M.splice(i,1)
  }
 }
}
...
function shockwave(){
 EX.push({x:pl.x,y:pl.y,radius:0,maxRadius:260,life:450,duration:450,color:"#fff07a"});
 ...
 for(const e of E){
  ...
  let resist=e.type==="mega"?.35:e.type==="tank"?.5:e.type==="big"?.7:1,f=(1-d/260);
 ...
 for(let i=0;i<40;i++){
  let A=Math.random()*Math.PI*2,S=160+Math.random()*380;
  PT.push({x:pl.x,y:pl.y,vx:Math.cos(A)*S,vy:Math.sin(A)*S,radius:2+Math.random()*3,life:300,color:"#fff07a",gravity:0})
 }
}
...
function upp(dt){
 for(let i=WP.length-1;i>=0;i--){
  ...
  if(hit(pl,q)){
   pl.weaponKey=q.weaponKey;weaponTime=30000;ls=0;WP.splice(i,1)
  }
 }
...
function uc(dt){
 ...
 for(let i=C.length-1;i>=0;i--){
  ...
  else if(hit(pl,q)){gain(q.value);C.splice(i,1)}
 }
}
...
function bg(){
 x.fillStyle="#111820";x.fillRect(0,0,W,H);x.strokeStyle="#ffffff08";
 for(let a=0;a<W;a+=50){x.beginPath();x.moveTo(a,0);x.lineTo(a,H);x.stroke()}
 for(let b=0;b<H;b+=50){x.beginPath();x.moveTo(0,b);x.lineTo(W,b);x.stroke()}
}

function dp(){
 if(surge>0){
  x.save();x.shadowBlur=30;x.shadowColor="#ffd700";x.strokeStyle="#ffd700";x.lineWidth=4;
  x.beginPath();x.arc(pl.x,pl.y,pl.radius+17+Math.sin(performance.now()/70)*4,0,Math.PI*2);x.stroke();x.restore()
 }

 if(ghost>0){
  x.save();x.globalAlpha=.35;x.shadowBlur=25;x.shadowColor="#d8deff";x.beginPath();x.arc(pl.x,pl.y,pl.radius+10,0,Math.PI*2);x.fillStyle="#d8deff";x.fill();x.restore()
 }

 if(deadeye>0){
  x.save();x.strokeStyle="#ff934d";x.lineWidth=2;x.beginPath();x.arc(pl.x,pl.y,pl.radius+13,0,Math.PI*2);x.stroke();x.restore()
 }
...
function dee(){
 for(const e of E){
  ...
  if(z.outline){
   ...
   x.beginPath();x.arc(e.x,e.y,e.radius+7,0,Math.PI*2);x.stroke();

   if(e.type==="mega"||e.type==="tank"){
    x.globalAlpha=.35;x.lineWidth=2;x.beginPath();x.arc(e.x,e.y,e.radius+14,0,Math.PI*2);x.stroke()
   }
   x.restore()
  }

  if(freeze>0){
   x.save();x.globalAlpha=.4;x.fillStyle="#78dcff";x.beginPath();x.arc(e.x,e.y,e.radius+3,0,Math.PI*2);x.fill();x.restore()
  }
  ...
  if(e.elite||e.health<e.maxHealth){
   let w=e.radius*1.8;x.fillStyle="#250000";x.fillRect(e.x-w/2,e.y-e.radius-10,w,5);
...
function dc(){
 for(const q of C){
  x.save();x.shadowBlur=q.nukeVacuum?16:7;x.shadowColor="#ffd700";x.fillStyle="#ffd700";
  x.beginPath();x.arc(q.x,q.y,q.radius,0,Math.PI*2);x.fill();x.restore()
 }
}
...
function db(){
 for(const b of B){
  ...
  if(b.visual==="tracer"){
   x.translate(b.x,b.y);x.rotate(b.angle);x.fillStyle="#fff";x.fillRect(-12,-2,24,4)
  }

  else if(b.visual==="plasma"){
   x.translate(b.x,b.y);x.rotate(b.angle);
   x.fillStyle=b.color;x.fillRect(-16,-3,32,6);
   x.globalAlpha=.35;x.fillRect(-24,-1,48,2)
  }

  else if(b.visual==="rail"){
   x.translate(b.x,b.y);x.rotate(b.angle);
   x.fillStyle="#fff";x.fillRect(-36,-2,72,4);
   x.globalAlpha=.4;x.fillStyle=b.color;x.fillRect(-48,-5,96,10)
  }

  else if(b.visual==="frost"){
   x.translate(b.x,b.y);x.rotate(b.angle);x.fillStyle=b.color;
   x.beginPath();x.moveTo(12,0);x.lineTo(2,-7);x.lineTo(-3,-3);x.lineTo(-10,-8);x.lineTo(-6,0);x.lineTo(-10,8);x.lineTo(-3,3);x.lineTo(2,7);x.closePath();x.fill()
  }

  else if(b.visual==="spark"){
   x.translate(b.x,b.y);x.rotate(b.angle);x.fillStyle="#fff2a3";
   x.beginPath();x.moveTo(10,0);x.lineTo(-5,-4);x.lineTo(-2,0);x.lineTo(-5,4);x.fill()
  }

  else if(b.visual==="comet"){
   x.fillStyle=b.color;x.beginPath();x.arc(b.x,b.y,b.radius,0,Math.PI*2);x.fill();
   x.globalAlpha=.35;
   for(let i=1;i<=3;i++){x.beginPath();x.arc(b.x-b.vx*.008*i,b.y-b.vy*.008*i,b.radius*(1-i*.18),0,Math.PI*2);x.fill()}
  }

  else if(b.visual==="flame"){
   let q=Math.max(0,b.life/900);
   x.globalAlpha=.55+.4*q;x.beginPath();x.arc(b.x,b.y,b.radius*(1.4+(1-q)*.9),0,Math.PI*2);
   x.fillStyle=q>.65?"#fff36e":q>.3?"#ff8a00":"#df2900";x.fill()
  }

  else if(b.visual==="singularity"){
   x.beginPath();x.arc(b.x,b.y,b.radius+3,0,Math.PI*2);x.fillStyle="#020005";x.fill();
   x.strokeStyle="#b98cff";x.lineWidth=3;x.stroke();
   x.globalAlpha=.3;x.beginPath();x.arc(b.x,b.y,b.radius+9+Math.sin(performance.now()/70)*3,0,Math.PI*2);x.stroke()
  }

  else if(b.visual==="void"){
   x.beginPath();x.arc(b.x,b.y,b.radius,0,Math.PI*2);x.fillStyle="#16002c";x.fill();
   x.lineWidth=4;x.strokeStyle="#a765ff";x.stroke();
   x.globalAlpha=.25;x.beginPath();x.arc(b.x,b.y,b.radius+8,0,Math.PI*2);x.stroke()
  }

  else if(b.visual==="lance"||b.visual==="lanceSide"){
   x.translate(b.x,b.y);x.rotate(b.angle);
   x.fillStyle=b.color;x.beginPath();x.moveTo(18,0);x.lineTo(-9,-4);x.lineTo(-5,0);x.lineTo(-9,4);x.fill()
  }

  else if(b.visual==="prism"||b.visual==="prismShard"){
   x.translate(b.x,b.y);x.rotate(b.angle);x.fillStyle=b.color;
   x.beginPath();x.moveTo(14,0);x.lineTo(-8,-6);x.lineTo(-5,0);x.lineTo(-8,6);x.closePath();x.fill();
   x.globalAlpha=.35;x.strokeStyle="#fff";x.stroke()
  }
...
function ds(){
 let n=UL("solar");if(!n)return;
 let count=Math.min(5,n+1),r=n>=5?82:70+n*2;

 for(let i=0;i<count;i++){
  let a=orb+Math.PI*2/count*i,A=pl.x+Math.cos(a)*r,Bb=pl.y+Math.sin(a)*r;
  x.save();x.translate(A,Bb);x.rotate(a+.35);x.fillStyle="#fff7a0";x.shadowBlur=18;x.shadowColor="#ffda44";
  x.beginPath();x.moveTo(0,-18);x.lineTo(6,7);x.lineTo(0,16);x.lineTo(-6,7);x.closePath();x.fill();x.restore()
 }
}
...
function dwp(){
 for(const p of WP){
  let w=G[p.weaponKey];
  x.save();x.shadowBlur=18;x.shadowColor=w.color;
  x.beginPath();x.arc(p.x,p.y,p.radius,0,Math.PI*2);x.fillStyle="#111";x.fill();
  x.strokeStyle=w.color;x.lineWidth=4;x.stroke();
  x.fillStyle="#fff";x.font="bold 8px Arial";x.textAlign="center";x.fillText(w.name.split(" ")[0],p.x,p.y+3);x.restore()
 }
}
...
function dpu(){
 for(const p of P){
  ...
  x.save();x.shadowBlur=["surge","nuke","berserk"].includes(p.key)?30:16;x.shadowColor=u.color;
  x.beginPath();x.arc(p.x,p.y,p.radius*q,0,Math.PI*2);x.fillStyle=u.color;x.fill();

  if(p.key==="ghost"){x.globalAlpha=.45;x.beginPath();x.arc(p.x,p.y,p.radius+7,0,Math.PI*2);x.strokeStyle="#fff";x.stroke()}
  if(p.key==="deadeye"){x.strokeStyle="#5c1900";x.lineWidth=2;x.beginPath();x.arc(p.x,p.y,8,0,Math.PI*2);x.stroke();x.beginPath();x.moveTo(p.x-12,p.y);x.lineTo(p.x+12,p.y);x.moveTo(p.x,p.y-12);x.lineTo(p.x,p.y+12);x.stroke()}

  x.globalAlpha=1;x.fillStyle="#111";x.font="bold 8px Arial";x.textAlign="center";x.fillText(u.name,p.x,p.y+3);x.restore()
 }
}
...
function dmet(){
 for(const m of M){
  ...
  x.save();x.strokeStyle=m.mega?"#fff2a8":"#ff6b35";x.lineWidth=m.mega?8:4;
  x.beginPath();x.arc(m.x,m.y,m.radius*(.65+.35*(1-q)),0,Math.PI*2);x.stroke();
  x.beginPath();x.moveTo(m.x-80*q,m.y-140*q);x.lineTo(m.x,m.y);x.stroke();x.restore()
 }
}
...
function dbh(){
 for(const q of BH){
  let p=1-q.life/q.duration;
  x.save();x.shadowBlur=35;x.shadowColor="#6d3cff";
  x.beginPath();x.arc(q.x,q.y,18+15*p,0,Math.PI*2);x.fillStyle="#020003";x.fill();
  x.strokeStyle="#b98cff";x.lineWidth=4;x.stroke();
  x.globalAlpha=.16;x.beginPath();x.arc(q.x,q.y,q.pullRadius,0,Math.PI*2);x.stroke();x.restore()
 }
}

function dfx(){
 for(const p of PT){
  x.save();x.globalAlpha=Math.min(1,p.life/250);x.beginPath();x.arc(p.x,p.y,p.radius,0,Math.PI*2);x.fillStyle=p.color;x.fill();x.restore()
 }

 for(const l of L){
  x.save();x.strokeStyle=l.color;x.shadowBlur=14;x.shadowColor=l.color;x.lineWidth=4;
  x.beginPath();x.moveTo(l.x1,l.y1);
  for(let i=1;i<6;i++){let q=i/6;x.lineTo(l.x1+(l.x2-l.x1)*q+(Math.random()-.5)*14,l.y1+(l.y2-l.y1)*q+(Math.random()-.5)*14)}
  x.lineTo(l.x2,l.y2);x.stroke();x.restore()
 }

 for(const e of EX){
  x.save();x.globalAlpha=Math.max(0,e.life/e.duration);x.strokeStyle=e.color;x.lineWidth=8;
  x.beginPath();x.arc(e.x,e.y,e.radius,0,Math.PI*2);x.stroke();x.restore()
 }
}
...
function hud(){
 ...
 st.textContent=`Score ${score}`;lvT.textContent=`LV ${lv}`;
 xpB.style.width=`${Math.min(100,xp/need()*100)}%`;

 if(pl.shield>0){shHUD.classList.remove("hidden");shT.textContent=Math.ceil(pl.shield)}
...
function reset(){
 ...
 score=kills=xp=pending=0;lv=1;t=0;
 ...
 run=1;choosing=0;jactive=0;jx=jy=0;
 jb.style.display="none";go.classList.add("hidden");lscr.classList.add("hidden");

 sw();lt=performance.now()
}
rb.onclick=reset;
