...
const G={
pistol:{name:"Pistol",type:"normal",damage:42,fireRate:350,bulletSpeed:800,bulletSize:6,color:"#fff",tier:0},
plasma:{name:"Plasma SMG",type:"normal",damage:22,fireRate:80,bulletSpeed:850,bulletSize:5,color:"#00eaff",tier:1},
frost:{name:"Frost Repeater",type:"frost",damage:28,fireRate:115,bulletSpeed:760,bulletSize:6,slow:.5,slowTime:2500,color:"#9fe8ff",tier:1},
railgun:{name:"Railgun",type:"railgun",damage:155,fireRate:850,bulletSpeed:1600,bulletSize:7,pierce:7,color:"#e600ff",tier:1},
shotgun:{name:"Titan Shotgun",type:"shotgun",damage:38,fireRate:620,bulletSpeed:720,bulletSize:7,pellets:9,spread:.8,knockback:50,color:"#ff9d00",tier:2},
starfire:{name:"Starfire Wand",type:"homing",damage:58,fireRate:250,bulletSpeed:540,bulletSize:8,homingStrength:7,color:"#ff4fd8",tier:2},
flame:{name:"Flamethrower",type:"flame",damage:14,fireRate:52,bulletSpeed:560,bulletSize:9,spread:.75,burnDamage:8,burnTime:1900,color:"#ff5a1f",tier:2},
singularity:{name:"Singularity Gun",type:"singularity",damage:0,fireRate:1250,bulletSpeed:420,bulletSize:15,pullRadius:300,pullStrength:520,blastRadius:205,blastDamage:145,blackHoleTime:700,color:"#6d3cff",tier:3},
solar:{name:"Solar Blades",type:"orbit",blades:7,color:"#ffda44",tier:3},
nova:{name:"Nova Lance",type:"nova",damage:75,fireRate:420,bulletSpeed:900,bulletSize:8,sideShots:2,color:"#ff6bff",tier:3},
void:{name:"Void Cannon",type:"explosive",damage:95,fireRate:1000,bulletSpeed:475,bulletSize:14,explosionRadius:145,explosionDamage:115,color:"#8c52ff",tier:4},
arc:{name:"Arc Blaster",type:"arc",damage:72,fireRate:630,chainDamage:55,chains:8,chainRange:225,color:"#66ffff",tier:4},
prism:{name:"Prism Cannon",type:"prism",damage:82,fireRate:650,bulletSpeed:1050,bulletSize:7,splits:3,splitRange:260,color:"#ff74e8",tier:5}
};

const U={
heal:{name:"HEAL",color:"#40ff75",w:20,d:0},
shield:{name:"SHIELD",color:"#5ac8ff",w:18,d:0},
magnet:{name:"MAGNET",color:"#c451ff",w:15,d:20000,m:2},
haste:{name:"HASTE",color:"#ffe600",w:12,d:18000,m:2},
berserk:{name:"BERSERK",color:"#ff0055",w:10,d:17000,m:2},
vampire:{name:"VAMPIRE",color:"#a80038",w:9,d:22000,m:2},
fmj:{name:"FMJ",color:"#c8c8c8",w:9,d:22000,m:2},
ricochet:{name:"RICOCHET",color:"#b6ff5c",w:7,d:22000,m:2},
quad:{name:"DAMAGE",color:"#ff3838",w:7,d:16000,m:2},
chain:{name:"CHAIN",color:"#00ffff",w:7,d:20000,m:2},
meteors:{name:"METEORS",color:"#ff6b35",w:5,d:20000,m:2},
overdrive:{name:"OVERDRIVE",color:"#ff7b00",w:4,d:16000,m:2},
lightning:{name:"LIGHTNING",color:"#68f7ff",w:4,d:20000,m:2},
multishot:{name:"MULTI",color:"#ff8cff",w:3,d:22000,m:2},
chainreaction:{name:"REACTION",color:"#ff3c6f",w:3,d:22000,m:2},
nuke:{name:"NUKE",color:"#fff",w:1.8,d:0},
powersurge:{name:"POWER SURGE",color:"#fff",w:.8,d:5500,m:1}
};
...
function weaponWeights(){let late=Math.min(1,t/180+score/5000),pity=Math.min(1,badWeaponRolls/6);return Object.entries(G).filter(([k])=>k!=="pistol").map(([k,g])=>{let w=1;if(g.tier===1)w=34-14*late;if(g.tier===2)w=28-6*late;if(g.tier===3)w=16+8*late;if(g.tier===4)w=7+11*late;if(g.tier===5)w=2+8*late;w*=1+(g.tier>=3?pity*1.6:0);if(badWeaponRolls>=6&&g.tier<3)w=0;return{k,w}})}
...
function se(){let s=Math.floor(Math.random()*4),m=50,a,b;if(!s){a=Math.random()*W;b=-m}else if(s===1){a=W+m;b=Math.random()*H}else if(s===2){a=Math.random()*W;b=H+m}else{a=-m;b=Math.random()*H}let q=1+score/700+t/110,r=19+Math.random()*4,v=70+Math.random()*28+q*3,dx=pl.x-a,dy=pl.y-b,d=Math.hypot(dx,dy)||1,h=70+q*12;E.push({x:a,y:b,radius:r,vx:dx/d*v*.7,vy:dy/d*v*.7,baseMaxSpeed:v,steering:120+Math.random()*35,health:h,maxHealth:h,damage:17+q*.45,hitCooldown:0,slowTimer:0,burnTime:0,burnTick:0,burnDamage:0,dead:0,bladeHit:0})}
...
function fn(src=pl,ex=new Set(),vis=1){let z=null,bd=1e9;for(const e of E){if(e.dead||ex.has(e)||(vis&&!ons(e)))continue;let d=dd(src,e);if(d<bd){bd=d;z=e}}return z}
...
function hp(a,b){for(let i=0;i<3;i++)PT.push({x:a,y:b,vx:(Math.random()-.5)*150,vy:(Math.random()-.5)*150,radius:2+Math.random()*2,life:180,color:"#fff",gravity:0})}
...
function de(e,d,k=0,a=0){if(!e||e.dead)return;e.health-=d*dm();if(k){e.vx+=Math.cos(a)*k*2.5;e.vy+=Math.sin(a)*k*2.5}hp(e.x,e.y);if(e.health<=0)ke(e)}

function chain(s,d,n,r,col,ex=new Set()){let f={x:s.x,y:s.y},Hh=new Set(ex);Hh.add(s);for(let i=0;i<n;i++){let q=null,bd=1e9;for(const e of E){if(e.dead||Hh.has(e)||!ons(e))continue;let z=Math.hypot(e.x-f.x,e.y-f.y);if(z<r&&z<bd){bd=z;q=e}}if(!q)break;L.push({x1:f.x,y1:f.y,x2:q.x,y2:q.y,life:130,color:col});de(q,d);Hh.add(q);f={x:q.x,y:q.y};d*=.86}}

function cb(a,w,o={}){B.push({x:o.x??pl.x,y:o.y??pl.y,vx:Math.cos(a)*(o.speed??w.bulletSpeed),vy:Math.sin(a)*(o.speed??w.bulletSpeed),angle:a,radius:o.radius??w.bulletSize,damage:o.damage??w.damage,color:o.color??w.color,type:o.type??w.type,pierce:(o.pierce??w.pierce??0)+es("fmj")*2,hitEnemies:new Set(),explosionRadius:o.explosionRadius??w.explosionRadius??0,explosionDamage:o.explosionDamage??w.explosionDamage??0,homingStrength:o.homingStrength??w.homingStrength??0,slow:w.slow||0,slowTime:w.slowTime||0,knockback:w.knockback||0,pullRadius:w.pullRadius||0,pullStrength:w.pullStrength||0,burnDamage:w.burnDamage||0,burnTime:w.burnTime||0,splitRange:w.splitRange||0,splits:w.splits||0,ricochets:es("ricochet"),life:o.life??(w.type==="singularity"?2200:3000)})}

function fa(e,w){let q=e,d=w.damage,f={x:pl.x,y:pl.y},h=new Set(),n=w.chains+es("overdrive")+es("chain");for(let i=0;i<n&&q;i++){if(!ons(q))break;L.push({x1:f.x,y1:f.y,x2:q.x,y2:q.y,life:140,color:w.color});de(q,d);h.add(q);f={x:q.x,y:q.y};d=i?d*.94:w.chainDamage;let z=null,bd=1e9;for(const p of E){if(p.dead||h.has(p)||!ons(p))continue;let u=Math.hypot(p.x-f.x,p.y-f.y);if(u<w.chainRange+es("chain")*25&&u<bd){bd=u;z=p}}q=z}}

function shoot(now){let w=gw();if(w.type==="orbit")return;let q=fn();if(!q||now-ls<fr(w))return;ls=now;let a=Math.atan2(q.y-pl.y,q.x-pl.x),m=es("multishot");if(w.type==="shotgun"){for(let i=0;i<w.pellets+m*2;i++)cb(a+(Math.random()-.5)*w.spread,w);return}if(w.type==="arc"){fa(q,w);return}if(w.type==="flame"){for(let i=0;i<2+m;i++)cb(a+(Math.random()-.5)*w.spread,w,{type:"flame",life:850,speed:w.bulletSpeed*(.85+Math.random()*.35),radius:w.bulletSize+Math.random()*5});return}if(w.type==="nova"){cb(a,w);let n=w.sideShots+es("overdrive")+m;for(let i=1;i<=n;i++){cb(a-.18*i,w);cb(a+.18*i,w)}return}cb(a,w);let n=es("overdrive")+m;for(let i=1;i<=n;i++){cb(a-.1*i,w);cb(a+.1*i,w)}}

function ps(b,e){if(b.type!=="prism"||!b.splits)return;let a=E.filter(q=>!q.dead&&q!==e&&ons(q)&&Math.hypot(q.x-e.x,q.y-e.y)<b.splitRange).sort((q,z)=>dd(e,q)-dd(e,z)).slice(0,b.splits+es("multishot"));for(const q of a)cb(Math.atan2(q.y-e.y,q.x-e.x),G.prism,{x:e.x,y:e.y,damage:b.damage*.65,type:"prismShard",speed:950,radius:5,life:900})}

function ric(b,e){if(b.ricochets<=0)return 0;let q=fn(e,new Set(b.hitEnemies),1);if(!q)return 0;let a=Math.atan2(q.y-e.y,q.x-e.x),s=Math.hypot(b.vx,b.vy);b.x=e.x;b.y=e.y;b.angle=a;b.vx=Math.cos(a)*s;b.vy=Math.sin(a)*s;b.ricochets--;return 1}

function boom(a,b,r,d,du=300,col="#c45cff"){EX.push({x:a,y:b,radius:0,maxRadius:r,life:du,duration:du,color:col});for(const e of E){if(e.dead)continue;let q=Math.hypot(e.x-a,e.y-b);if(q<r)de(e,d*(.4+(1-q/r)*.6))}}

function singularity(a,b){
 let w=G.singularity;
 BH.push({x:a,y:b,life:w.blackHoleTime,duration:w.blackHoleTime,pullRadius:w.pullRadius,pullStrength:w.pullStrength,blastRadius:w.blastRadius,blastDamage:w.blastDamage,color:w.color,particleTimer:0});
 for(let i=0;i<28;i++){
  let A=Math.random()*Math.PI*2,R=55+Math.random()*w.pullRadius*.85;
  PT.push({x:a+Math.cos(A)*R,y:b+Math.sin(A)*R,vx:0,vy:0,radius:1.5+Math.random()*2.5,life:w.blackHoleTime+100,color:Math.random()<.5?"#a574ff":"#fff",gravity:0,suckX:a,suckY:b,suckSpeed:250+Math.random()*250,suck:1,angle:A,spin:(Math.random()<.5?-1:1)*(2+Math.random()*4)});
 }
}
...
function singularityBoom(q){
 ...
 for(let i=0;i<45;i++){
  let A=Math.random()*Math.PI*2,S=160+Math.random()*500;
  PT.push({x:q.x,y:q.y,vx:Math.cos(A)*S,vy:Math.sin(A)*S,radius:2+Math.random()*4,life:350+Math.random()*350,color:Math.random()<.5?"#d7b8ff":"#7b33ff",gravity:0});
 }
 for(const e of E){
  if(e.dead)continue;
  let dx=e.x-q.x,dy=e.y-q.y,d=Math.hypot(dx,dy)||1;
  if(d<q.blastRadius*1.35){let f=220*(1-Math.min(1,d/(q.blastRadius*1.35)));e.vx+=dx/d*f;e.vy+=dy/d*f}
 }
}
...
function ue(dt){for(let i=E.length-1;i>=0;i--){let e=E[i];if(e.dead){E.splice(i,1);continue}let ms=e.baseMaxSpeed;if(e.slowTimer>0){e.slowTimer-=dt*1000;ms*=.5}if(e.burnTime>0){e.burnTime-=dt*1000;e.burnTick-=dt*1000;if(e.burnTick<=0){e.burnTick=250;de(e,e.burnDamage)}}if(e.dead)continue;let dx=pl.x-e.x,dy=pl.y-e.y,d=Math.hypot(dx,dy)||1,vx=dx/d*ms,vy=dy/d*ms,sx=vx-e.vx,sy=vy-e.vy,sl=Math.hypot(sx,sy),mx=e.steering*dt;if(sl>mx&&sl){sx=sx/sl*mx;sy=sy/sl*mx}e.vx+=sx;e.vy+=sy;let v=Math.hypot(e.vx,e.vy);if(v>ms){e.vx=e.vx/v*ms;e.vy=e.vy/v*ms}e.x+=e.vx*dt;e.y+=e.vy*dt;e.hitCooldown-=dt*1000;if(dtm<=0&&hit(pl,e)&&e.hitCooldown<=0){let z=e.damage;if(pl.shield>0){let a=Math.min(pl.shield,z);pl.shield-=a;z-=a}pl.health-=z;e.hitCooldown=650;e.vx-=dx/d*90;e.vy-=dy/d*90;if(pl.health<=0){pl.health=0;end()}}}}

function ub(dt){
 for(let i=B.length-1;i>=0;i--){
  let b=B[i];b.life-=dt*1000;
  if(b.type==="homing"){let q=fn(b,b.hitEnemies,1);if(q){let a=Math.atan2(q.y-b.y,q.x-b.x),df=Math.atan2(Math.sin(a-b.angle),Math.cos(a-b.angle));b.angle+=df*b.homingStrength*dt;let s=Math.hypot(b.vx,b.vy);b.vx=Math.cos(b.angle)*s;b.vy=Math.sin(b.angle)*s}}
  ...
  for(const e of E){
   ...
   de(e,b.damage,b.knockback,b.angle);
...
function ubh(dt){
 for(let i=BH.length-1;i>=0;i--){
  let q=BH[i];q.life-=dt*1000;q.particleTimer-=dt*1000;
  let progress=1-q.life/q.duration,strength=.35+progress*.9;
  for(const e of E){
   if(e.dead)continue;
   let dx=q.x-e.x,dy=q.y-e.y,d=Math.hypot(dx,dy);
   if(d>2&&d<q.pullRadius){
    let f=q.pullStrength*(1-d/q.pullRadius)*strength*dt;
    e.vx+=dx/d*f;e.vy+=dy/d*f;
    e.x+=dx/d*f*.18;e.y+=dy/d*f*.18;
   }
  }
  if(q.particleTimer<=0){
   q.particleTimer=35;
   for(let z=0;z<3;z++){
    let A=Math.random()*Math.PI*2,R=80+Math.random()*q.pullRadius*.8;
    PT.push({x:q.x+Math.cos(A)*R,y:q.y+Math.sin(A)*R,vx:0,vy:0,radius:1+Math.random()*2.5,life:500,color:Math.random()<.5?"#8d55ff":"#eee4ff",gravity:0,suckX:q.x,suckY:q.y,suckSpeed:350+Math.random()*350,suck:1,angle:A,spin:(Math.random()<.5?-1:1)*(3+Math.random()*4)});
   }
  }
...
function ul(dt){let s=es("lightning");if(!s)return;lgt+=dt*1000;let r=Math.max(600,1500-s*200);if(lgt<r)return;lgt=0;let a=E.filter(e=>!e.dead&&ons(e));if(!a.length)return;let q=a[Math.floor(Math.random()*a.length)],d=45+s*20;L.push({x1:q.x+(Math.random()-.5)*50,y1:-20,x2:q.x,y2:q.y,life:180,color:"#68f7ff"});de(q,d);chain(q,d*.7,1+s*2,150+s*35,"#68f7ff")}
 ...
 for(let i=PT.length-1;i>=0;i--){
  ...
  if(p.suck){
   ...
   if(d>2){
    ...
    p.angle+=p.spin*dt;
    let s=p.suckSpeed*(1+Math.max(0,1-d/300)*1.5);
...
function db(){for(const b of B){x.save();x.shadowBlur=b.type==="singularity"?28:12;x.shadowColor=b.color;x.beginPath();x.arc(b.x,b.y,Math.max(0,b.radius),0,Math.PI*2);x.fillStyle=b.type==="singularity"?"#140025":b.color;x.fill();x.strokeStyle=b.color;x.lineWidth=b.type==="singularity"?4:0;if(b.type==="singularity")x.stroke();x.restore()}}

function ds(){let w=gw();if(w.type!=="orbit")return;let n=w.blades+es("overdrive")+es("multishot");for(let i=0;i<n;i++){let a=orb+Math.PI*2/n*i,A=pl.x+Math.cos(a)*72,Bb=pl.y+Math.sin(a)*72;x.save();x.translate(A,Bb);x.rotate(a);x.shadowBlur=20;x.shadowColor=w.color;x.fillStyle=w.color;x.fillRect(-6,-22,12,44);x.restore()}}
...
function dbh(){
 for(const q of BH){
  let p=1-q.life/q.duration,r=q.pullRadius*(.35+.65*p);
  x.save();
  x.globalAlpha=.35+.35*Math.sin(performance.now()/45);
  x.shadowBlur=35;x.shadowColor=q.color;
  x.beginPath();x.arc(q.x,q.y,18+12*p,0,Math.PI*2);x.fillStyle="#050008";x.fill();x.strokeStyle="#b98cff";x.lineWidth=3;x.stroke();
  x.globalAlpha=.2;
  x.beginPath();x.arc(q.x,q.y,r,0,Math.PI*2);x.strokeStyle="#8c52ff";x.lineWidth=2;x.stroke();
  x.restore();
 }
}

function dfx(){for(const p of PT){x.save();x.globalAlpha=Math.min(1,p.life/250);x.beginPath();x.arc(p.x,p.y,p.radius,0,Math.PI*2);x.fillStyle=p.color||"#fff";x.fill();x.restore()}for(const e of EX){x.save();x.globalAlpha=Math.max(0,e.life/e.duration);x.beginPath();x.arc(e.x,e.y,e.radius,0,Math.PI*2);x.strokeStyle=e.color;x.lineWidth=10;x.stroke();x.restore()}for(const l of L){x.save();x.shadowBlur=15;x.shadowColor=l.color;x.strokeStyle=l.color;x.lineWidth=4;x.beginPath();x.moveTo(l.x1,l.y1);for(let i=1;i<5;i++){let q=i/5;x.lineTo(l.x1+(l.x2-l.x1)*q+(Math.random()-.5)*15,l.y1+(l.y2-l.y1)*q+(Math.random()-.5)*15)}x.lineTo(l.x2,l.y2);x.stroke();x.restore()}}
...
function loop(n){
 ...
 if(run){
  ...
  let alive=E.reduce((q,e)=>q+(!e.dead),0),er=Math.max(110,1200-score*.55-t*1.5),ec=score>=12000?4:score>=6500?3:score>=2500?2:1;
